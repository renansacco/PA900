...
odefun = @(t, X) malha_fechada(t, X, p, vx, K_psi, K_r, K_delta, gamma0, cfg.omega_sat);
...
for i = 1:n
    ...
    r_i     = Xout(i, 4);
    delta_i = Xout(i, 7);
    u_psi = K_psi * e_psi + K_r * (0 - r_i) + K_delta * (0 - delta_i);
...
if cfg.plot
    ...
    fprintf('  J_base:               %.4f\n', J_base);
    fprintf('  J_ms  (robustez):     %.4f  (Ms=%.2f)\n', J_ms, Ms);
    fprintf('  J_bw  (banda):        %.4f  (tau_MF=%.3fs)\n', J_bw, tau_mf);
    fprintf('  J_total:              %.4f\n', J);
end
...
if cfg.plot
    ...
    sgtitle(sprintf('K=[%.1f, %.1f, %.1f]  J=%.4f  vx=%.1f  \\gamma_0=%.1f°', ...
        K_psi, K_r, K_delta, J, vx, rad2deg(gamma0)));
    drawnow;
end
...
%% -----------------------------------------------------------------------
function Xp = malha_fechada(t, X, p, vx, K_psi, K_r, K_delta, gamma0, omega_sat)
    psi = X(3); r = X(4); delta = X(7);

    psi_ref = gamma0;

    omega_m_ref = K_psi * (psi_ref - psi) + K_r * (0 - r) + K_delta * (0 - delta);
